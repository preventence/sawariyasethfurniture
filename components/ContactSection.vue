    ...
    <div class="max-w-7xl mx-auto px-6 lg:px-10">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-16">
        <div class="reveal">
          ...
          <div class="wood-rule" />
          <p class="font-sans text-sm text-[var(--color-bark)]/65 leading-relaxed mb-10">Whether you have a complete floor plan or just an idea on a napkin, our design team will work with you from brief to installation. Enquiries receive a response within 24 hours.</p>
          <div class="space-y-5">
            <a v-for="m in contactMethods" :key="m.label" :href="m.href" :target="m.external?'_blank':undefined" :rel="m.external?'noopener noreferrer':undefined" class="flex items-center gap-4 group">
        ...
        <div class="bg-white border border-[var(--color-linen)] p-8 lg:p-10 reveal">
          ...
          <form v-if="!submitted" @submit.prevent="submit" class="space-y-5" novalidate>
            ...
            <div><label class="form-label">Tell Us More</label><textarea v-model="form.message" class="form-input resize-none" rows="4" placeholder="Room dimensions, inspiration, timeline…" /></div>
...
.form-input{width:100%;font-family:var(--font-sans);font-size:.875rem;color:var(--color-bark);background:transparent;border:1px solid rgba(44,26,14,.18);padding:.65rem .875rem;outline:none;transition:border-color .2s ease;appearance:none;}
